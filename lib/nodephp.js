	...
	connection.addListener("connect", function() {
		...
		parser.onBody = function(buffer, start, end) {
            ...
            try {
                ...
                response.setHeader('X-Server' , 'Node.js-' + process.version);
 ...
 * @return void 
 */
function serveStatic(file, path, response) {
   ...
   var types = {
    ...
    try {
        response.writeHead(200, {'Content-Type': types[file.substr(file.indexOf('.'), file.length)]});
...
function createServer(opts, block) {
    ...
    http.createServer(function(request, response) {
        ...
        var extension = pathname.substr(-4);

        for (rule in block) {
            if (rule == 'index') { continue; }
            if (pathname.match(rule)) {
                if (block[rule] == NODEPHP_STATIC) {
                    return serveStatic(pathname, __script_dir__, response);
    ...
    console.log("PHP Server is now running on port " + options.server.port);
};
